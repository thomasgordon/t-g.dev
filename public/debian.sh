...
# Debian bootstrap (same as Ubuntu) — installs yadm, pulls dotfiles, and common tools.
# Usage: curl -fsSL https://t-g.dev/public/debian.sh | bash
...
install_base() {
  ...
  sudo apt-get install -y --no-install-recommends ca-certificates curl git wget gnupg lsb-release unzip zsh fzf jq neofetch build-essential
...
main() {
  ...
  install_zoxide
  set_zsh_default
  log "Debian bootstrap complete. Restart your shell or log out/in if needed."
}
